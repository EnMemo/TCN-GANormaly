...
from models.tools import csvread, check_dir, save_model, load_model, ls_adv_loss, mean_gen_loss, permute_results, txt_save, acc_save, test_save, loss_save, get_files
import warnings
warnings.filterwarnings("ignore") 
starttime = datetime.datetime.now()

parser = argparse.ArgumentParser(description='Arguments parser for TCN-GAN training')
parser.add_argument('-dx','--dim_x', type=int, default=1)
...
parser.add_argument('-db','--dataset_type', type=str, default='gear')
...
parser.add_argument('-threshold','--threshold', type=float, default=3.0)
...
list_data = get_files()
train_data_len = int(len(list_data[0][0])*args.data_use)
normal_data = (torch.from_numpy(list_data[0][0]).to(device)).type(torch.FloatTensor) 
train_data = normal_data[:train_data_len,:].to(device)  
...
for e in range(args.trails):
    ...
    if args.code_mode=='test_mode':  
        ...
        normal_test_data = normal_data[train_data_len:2*train_data_len,:].to(device)
        ...
        losses, time, flag = test_model(0, './output/%s/test_%s_e%d.txt'%(args.output, 'normal', e), test_loader, generator, discriminator, threshold)
        test_save('./output/%s/test_e%d.csv'%(args.output, e), '0.csv', losses['rec_loss'], time, flag)

        abnorm_cnt = 0.0
        abnorm_sum = 8
        for i in range(1,9):
            abnormal_data = (torch.from_numpy(list_data[0][i]).to(device)).type(torch.FloatTensor) 
            test_data = abnormal_data[:data_len,:].to(device) 
            test_db = DB(test_data, args.seqlen)
            test_loader = DataLoader(dataset=test_db, batch_size=args.batch_size, shuffle=False, drop_last=True)
            losses, time, flag = test_model(i, './output/%s/test_%s_e%d.txt'%(args.output, 'abnormal', e), test_loader, generator, discriminator, threshold)
            test_save('./output/%s/test_e%d.csv'%(args.output, e), '%d.csv'%i, losses['rec_loss'], time, flag)
            abnorm_cnt += flag
